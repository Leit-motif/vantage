...
Each mutation was applied **alone** on `ac4c4a9`, the named test run, and `src/` and `tests/`
restored from that commit before the next one. **Sixteen mutations, sixteen failures, no passes.**
The suite is green on the unmutated commit — `Vantage.Tests` 143/143 and `Vantage.Tests.Shell`
82/82.
...
| 16 | The committed side claims a timestamp again | `The_committed_side_carries_no_timestamp_borrowed_from_another_branch` | failed |
...
Rows 12 to 16 hold the five behaviours added in response to the cold review and the acceptance
audit, so a fix made for a finding is measured the same way as the original work.

## Three mutations that had to be rewritten

All three are recorded, because a mutation that fails to break anything is the same mistake as a
test that fails to check anything — and none of them is visible from a green run.

**Constant conditions do not compile here.** Three mutations were first written as `if (false)` or an
early `return null`. This build treats warnings as errors, so the unreachable code that follows fails
the build and no test can run. They were rewritten to mutate a value rather than a constant, which is
the stronger form anyway: the code still runs, it just runs wrong.
...
both expressions are true and the mutant behaves exactly like the original — the test passed, and the
run was right to. Removing the clause outright is the mutation that actually weakens the guard, and
the test fails on it.

**Row 16's first attempt never reached the file.** Its pattern spanned two lines with a bare `\n`,
and this working tree is checked out with CRLF endings, so it matched nothing: `perl` exited 0, the
source was untouched, and the test passed against unmutated code. Rewritten with `\s*` between the
two lines, it applies and the test fails.

That one is the most dangerous of the three, because a mutation that silently fails to apply reports
exactly what a well-tested behaviour reports — a pass — and there is nothing in the output to tell
them apart. The general rule the three share: **a passing mutation is a claim about the mutation
before it is a claim about the test**, and the mutation has to be ruled out first.
